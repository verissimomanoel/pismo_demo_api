 ...
 */
package br.com.pismo.demo.api.auth.impl;
...
public class UserServiceImpl implements UserService {
    ...
    private void checkDuplicity(User user) {
        ...
        if (userOptional.isPresent() && (user.getId() == null || !user.getId().equals(userOptional.get().getId()))) {
            throw new BusinessException(PismoDemoMessageCode.ERROR_USER_ALREADY_EXISTS);
        }
    }
    ...
    public User update(User user) {
        if (user == null || StringUtils.isEmpty(user.getId())) {
            throw new BusinessException(PismoDemoMessageCode.ERROR_REQUIRED_FIELDS, "Id");
        }
    ...
    public List<User> search(String name) {
        if (StringUtils.isEmpty(name)) {
            throw new BusinessException(PismoDemoMessageCode.ERROR_FILTER_REQUIRED);
        }

        List<User> users = userRepository.findByName(name);
        if (users == null || users.isEmpty()) {
            throw new BusinessException(PismoDemoMessageCode.ERROR_NO_FOUND);
        }
